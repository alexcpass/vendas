...
<style>
    ...
    /* Botões dentro do Destaque (Estilo Glass) */
...
# -----------------------------------------------------------------------------
# 2. FUNÇÃO DE PROCESSAMENTO DE DADOS (COM TRATAMENTO DE ERROS)
# -----------------------------------------------------------------------------
@st.cache_data
def process_data(v_file, c_file, p_file):
    try:
        ...
        if 'ClienteID' not in vendas.columns or 'ProdutoID' not in vendas.columns:
            return None, "O arquivo de Vendas deve conter 'ClienteID' e 'ProdutoID'."

        # Merges
        df = vendas.merge(clientes, on='ClienteID', how='left').merge(produtos, on='ProdutoID', how='left')
        
        # Tratamento Monetário (Robustez para formatos BR/US)
...
# Aplicação Filtros Secundários
if cat_sel != 'Todas': df_filtered = df_filtered[df_filtered['Categoria'] == cat_sel]
if pag_sel != 'Todas': df_filtered = df_filtered[df_filtered['FormaPagamento'] == pag_sel]
...
# GRÁFICO 1: FATURAMENTO (Light)
st.markdown("### 📈 Evolução do Faturamento")
df_fat = df_filtered.groupby(['Mes', 'MesNome'])['ValorTotal'].sum().reset_index().sort_values('Mes')

fig_fat = px.bar(df_fat, x='MesNome', y='ValorTotal', text='ValorTotal', color_discrete_sequence=['#4169E1'])
...
fig_vol.add_trace(go.Bar(
    ...
    text=df_vol['VendaID'],
    textposition='inside',
    textfont=dict(color='black', weight='bold') # Texto interno preto para contraste com laranja
))
...
fig_vol.update_layout(
    ...
    xaxis=dict(
        showgrid=False, 
        tickfont=dict(color='white', size=13, weight='bold'),
        linecolor='#A0AEC0'
    ),
    yaxis=dict(
        showgrid=True, 
...
fig_prod = px.bar(df_top, y='NomeProduto', x='ValorTotal', orientation='h', text='ValorTotal', color='ValorTotal', color_continuous_scale='Blues')
fig_prod.update_traces(texttemplate='R$ %{text:,.0f}', textposition='outside', textfont=dict(color='#d32f2f', weight='bold'))
fig_prod.update_layout(height=450, plot_bgcolor='white', xaxis=dict(showgrid=True, gridcolor='#f1f5f9'), coloraxis_showscale=False)
