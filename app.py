...
with st.sidebar:
    ...
    vendedores_file = st.file_uploader("Vendedores.csv (opcional)", type=['csv'], key="vendedores")
    regioes_file = st.file_uploader("Regiões.csv (opcional)", type=['csv'], key="regioes")
    
    arquivos_obrigatorios = all([vendas_file, clientes_file, produtos_file])
    
    if arquivos_obrigatorios:
        total_arquivos = 3 + sum([bool(vendedores_file), bool(regioes_file)])
        st.success(f"✅ {total_arquivos}/5 arquivos carregados")
    else:
        faltam = 3 - sum([bool(vendas_file), bool(clientes_file), bool(produtos_file)])
        st.warning(f"⚠️ Faltam {faltam} arquivo(s) obrigatório(s)")
...
if not arquivos_obrigatorios:
    st.markdown("""
        <div style='text-align: center; padding: 80px 20px;'>
            ...
            <p style='font-size: 16px; color: #718096;'>Carregue os 3 arquivos obrigatórios na barra lateral</p>
        </div>
    """, unsafe_allow_html=True)
    st.stop()
...
# BOTÕES DE FILTRO DE ANO (ESTILO METABASE)
st.markdown("<p style='font-size: 12px; color: #718096; font-weight: 600; margin-bottom: 10px; text-transform: uppercase;'>DASHBOARDS</p>", unsafe_allow_html=True)
...
num_botoes = len(anos_disponiveis) + 2  # anos + "Todos" + filtros rápidos
cols_botoes = st.columns(num_botoes)

# Botão "Selecionar tudo"
with cols_botoes[0]:
    btn_type = "primary" if st.session_state.ano_selecionado == "Todos" else "secondary"
    if st.button("Selecionar tudo", use_container_width=True, type=btn_type, key="btn_todos"):
        st.session_state.ano_selecionado = "Todos"
        st.rerun()

# Botões de cada ano
for idx, ano in enumerate(anos_disponiveis):
    with cols_botoes[idx + 1]:
        btn_type = "primary" if st.session_state.ano_selecionado == ano else "secondary"
        if st.button(str(ano), use_container_width=True, type=btn_type, key=f"ano_{ano}"):
            st.session_state.ano_selecionado = ano
            st.rerun()

# Botões de filtros rápidos por categoria
categorias_unicas = sorted(df['Categoria'].unique().tolist())
for idx, cat in enumerate(categorias_unicas):
    with cols_botoes[len(anos_disponiveis) + 1 + idx]:
        if st.button(cat, use_container_width=True, key=f"cat_{cat}"):
            st.info(f"Filtro rápido: {cat}")
...
with st.sidebar:
    ...
    st.markdown(f"""
        <div style='padding: 15px; background: rgba(66, 153, 225, 0.1); border-radius: 8px;'>
            <p style='font-size: 11px; color: #a0aec0; margin: 0;'>PERÍODO DOS DADOS</p>
...
with col4:
    st.markdown(f"""
        <div style='background: white; padding: 20px; border-radius: 8px; box-shadow: 0 2px 8px rgba
